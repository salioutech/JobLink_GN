...
<div style="display:grid;grid-template-columns:220px 1fr;min-height:calc(100vh - 64px);">
    ...
    <div style="background:#fff;border-right:1px solid #E2E8F0;padding:20px 12px;display:flex;flex-direction:column;gap:4px;">
        <div style="text-align:center;padding:14px 8px;margin-bottom:12px;border-bottom:1px solid #E2E8F0;">
            ...
            <div style="font-size:13px;font-weight:700;color:#0D2137;">{{ $user->profile->nom ?? '' }}</div>
            <div style="font-size:11px;color:#5a6a7a;margin-bottom:8px;">Entreprise · {{ $user->profile->localisation ?? 'Conakry' }}</div>
        ...
        <a href="{{ route('profil.edit') }}" style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-radius:8px;font-size:14px;color:#5a6a7a;text-decoration:none;">🏢 Profil entreprise</a>
    ...
    <div style="padding:24px;background:#F4F6F9;overflow-y:auto;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
            <div>
                ...
                <p style="font-size:13px;color:#5a6a7a;">Gérez vos offres et suivez les candidatures</p>
            </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:24px;">
            ...
            <div style="background:#fff;border-radius:10px;border:1px solid #E2E8F0;padding:16px;text-align:center;border-top:3px solid #F0A500;">
                ...
                <div style="font-size:12px;color:#5a6a7a;margin-top:4px;">Candidatures reçues</div>
        ...
        <div style="background:#fff;border-radius:12px;border:1px solid #E2E8F0;padding:20px;margin-bottom:20px;">
                    ...
                    @foreach($offres as $offre)
                        <div style="background:#F4F6F9;border-radius:8px;padding:14px;border-left:3px solid {{ $offre->statut === 'active' ? '#1A4B7A' : '#ccc' }};">
                            ...
                            <div style="font-size:12px;color:#5a6a7a;margin-bottom:8px;">
                                ...
                                <strong style="color:#1A4B7A;">{{ $offre->candidatures_count ?? 0 }} candidature(s)</strong>
                            </div>
                            <div style="display:flex;gap:8px;">
                                ...
                                <a href="{{ route('candidature.received') }}"
                                    style="font-size:11px;padding:5px 12px;border-radius:6px;border:1px solid #E2E8F0;background:#fff;color:#1A4B7A;text-decoration:none;font-weight:600;">
                                    👥 Voir candidatures
                                </a>
                                <form method="POST" action="{{ route('offre.destroy', $offre->id) }}" style="display:inline;" onsubmit="return confirm('Supprimer cette offre ?')">
                                    @csrf @method('DELETE')
            ...
            @else
                <div style="text-align:center;padding:32px;color:#5a6a7a;">
                    ...
                    <div style="font-size:14px;font-weight:600;margin-bottom:6px;">Aucune offre publiée</div>
                    <a href="{{ route('offre.create') }}" style="background:#1A4B7A;color:#fff;padding:10px 20px;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none;">+ Publier une offre</a>
                </div>
            @endif
        </div>
