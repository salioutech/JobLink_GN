@extends('layouts.app')
@section('title', 'Dashboard Particulier')
...
<div style="display:grid;grid-template-columns:220px 1fr;min-height:calc(100vh - 64px);">
    ...
    <div style="background:#fff;border-right:1px solid #E2E8F0;padding:20px 12px;display:flex;flex-direction:column;gap:4px;">
        <div style="text-align:center;padding:14px 8px;margin-bottom:12px;border-bottom:1px solid #E2E8F0;">
            ...
            <div style="font-size:11px;color:#5a6a7a;margin-bottom:8px;">Particulier · {{ $user->profile->localisation ?? 'Conakry' }}</div>
        ...
        <a href="{{ route('offre.create') }}" style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-radius:8px;font-size:14px;color:#5a6a7a;text-decoration:none;">📋 Mes demandes</a>
        <a href="{{ route('demande.sent') }}" style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-radius:8px;font-size:14px;color:#5a6a7a;text-decoration:none;">🤝 Prestataires contactés</a>
        <a href="{{ route('search') }}" style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-radius:8px;font-size:14px;color:#5a6a7a;text-decoration:none;">🔍 Chercher un prestataire</a>
    ...
    <div style="padding:24px;background:#F4F6F9;overflow-y:auto;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
            <div>
                ...
                <p style="font-size:13px;color:#5a6a7a;">Retrouvez vos demandes publiées et vos prestataires contactés</p>
            </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:24px;">
            <div style="background:#fff;border-radius:10px;border:1px solid #E2E8F0;padding:16px;text-align:center;border-top:3px solid #1A4B7A;">
                ...
                <div style="font-size:12px;color:#5a6a7a;margin-top:4px;">Demandes publiées</div>
                <div style="font-size:11px;color:#1A9B5A;margin-top:2px;">{{ $offres->where('statut','active')->count() }} actives</div>
            </div>
            ...
            <div style="background:#fff;border-radius:10px;border:1px solid #E2E8F0;padding:16px;text-align:center;border-top:3px solid #1A9B5A;">
                <div style="font-size:28px;font-weight:700;color:#1A9B5A;">{{ $demandes->where('statut','acceptee')->count() }}</div>
                <div style="font-size:12px;color:#5a6a7a;margin-top:4px;">Réponses reçues</div>
                <div style="font-size:11px;color:#1A9B5A;margin-top:2px;">Acceptées</div>
            </div>
        </div>
        ...
        <div style="background:#fff;border-radius:12px;border:1px solid #E2E8F0;padding:20px;margin-bottom:20px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                <h2 style="font-size:15px;font-weight:700;color:#0D2137;">Mes demandes publiées</h2>
                <a href="{{ route('offre.create') }}" style="font-size:12px;color:#1A9B5A;font-weight:600;text-decoration:none;">+ Nouvelle demande</a>
            </div>

            @if($offres->count() > 0)
                <div style="display:flex;flex-direction:column;gap:10px;">
                    @foreach($offres as $offre)
                        <div style="background:#F4F6F9;border-radius:8px;padding:14px;border-left:3px solid {{ $offre->statut === 'active' ? '#1A4B7A' : '#ccc' }};">
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                                <div style="font-size:13px;font-weight:700;color:#0D2137;">{{ $offre->titre }}</div>
                                <span style="background:{{ $offre->statut === 'active' ? '#E8F0F9' : '#f0f0f0' }};color:{{ $offre->statut === 'active' ? '#1A4B7A' : '#888' }};padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;">
                            ...
                            <div style="display:flex;gap:8px;">
                                ...
                                <form method="POST" action="{{ route('offre.destroy', $offre->id) }}" style="display:inline;" onsubmit="return confirm('Supprimer cette demande ?')">
                                    @csrf @method('DELETE')
            ...
            @else
                <div style="text-align:center;padding:32px;color:#5a6a7a;">
                    ...
                    <div style="font-size:14px;font-weight:600;margin-bottom:6px;">Aucune demande publiée</div>
                    <a href="{{ route('offre.create') }}" style="background:#1A4B7A;color:#fff;padding:10px 20px;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none;">+ Publier une demande</a>
                </div>
            @endif
        </div>

        {{-- PRESTATAIRES CONTACTÉS --}}
        <div style="background:#fff;border-radius:12px;border:1px solid #E2E8F0;padding:20px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                <h2 style="font-size:15px;font-weight:700;color:#0D2137;">Prestataires contactés</h2>
                <a href="{{ route('demande.sent') }}" style="font-size:12px;color:#1A9B5A;font-weight:600;text-decoration:none;">Voir tout →</a>
            </div>
                    ...
                    @foreach($demandes->take(3) as $demande)
                        <div style="background:#F4F6F9;border-radius:8px;padding:14px;border-left:3px solid
                            ...
                            <div style="font-size:12px;color:#5a6a7a;margin-bottom:6px;">
                                {{ ucfirst($demande->offreur->role) }} · {{ $demande->created_at->diffForHumans() }}
                            </div>
            ...
            @else
                <div style="text-align:center;padding:24px;color:#5a6a7a;">
                    <div style="font-size:13px;margin-bottom:12px;">Vous n'avez encore contacté aucun prestataire.</div>
                    <a href="{{ route('search') }}"
                        style="background:#1A9B5A;color:#fff;padding:10px 20px;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none;">
