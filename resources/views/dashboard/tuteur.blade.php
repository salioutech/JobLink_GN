@extends('layouts.app')
@section('title', 'Dashboard Tuteur')
...
<div style="display:grid;grid-template-columns:220px 1fr;min-height:calc(100vh - 64px);">
    ...
    <div style="background:#fff;border-right:1px solid #E2E8F0;padding:20px 12px;display:flex;flex-direction:column;gap:4px;">
        <div style="text-align:center;padding:14px 8px;margin-bottom:12px;border-bottom:1px solid #E2E8F0;">
            <div style="width:52px;height:52px;border-radius:50%;background:#F0A500;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#0D2137;margin:0 auto 8px;border:3px solid #1A4B7A;">
            ...
            <div style="font-size:11px;color:#5a6a7a;margin-bottom:8px;">Tuteur · {{ $user->profile->localisation ?? 'Conakry' }}</div>
        ...
        <a href="{{ route('service.create') }}" style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-radius:8px;font-size:14px;color:#5a6a7a;text-decoration:none;">🎓 Mes cours</a>
        ...
        <a href="{{ route('demande.index') }}" style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-radius:8px;font-size:14px;color:#5a6a7a;text-decoration:none;">🤝 Demandes d'élèves</a>
    ...
    <div style="padding:24px;background:#F4F6F9;overflow-y:auto;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
            <div>
                ...
                <p style="font-size:13px;color:#5a6a7a;">Gérez vos cours et vos demandes d'élèves</p>
            </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:24px;">
            <div style="background:#fff;border-radius:10px;border:1px solid #E2E8F0;padding:16px;text-align:center;border-top:3px solid #F0A500;">
                ...
                <div style="font-size:12px;color:#5a6a7a;margin-top:4px;">Cours publiés</div>
            ...
            <div style="background:#fff;border-radius:10px;border:1px solid #E2E8F0;padding:16px;text-align:center;border-top:3px solid #1A9B5A;">
                <div style="font-size:28px;font-weight:700;color:#1A9B5A;">{{ $demandes->count() }}</div>
                <div style="font-size:12px;color:#5a6a7a;margin-top:4px;">Demandes d'élèves</div>
        ...
        <div style="background:#fff;border-radius:12px;border:1px solid #E2E8F0;padding:20px;margin-bottom:20px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                <h2 style="font-size:15px;font-weight:700;color:#0D2137;">🎓 Mes cours publiés</h2>
                <a href="{{ route('service.create') }}" style="font-size:12px;color:#1A9B5A;font-weight:600;text-decoration:none;">+ Nouveau cours</a>
            </div>
            @if($services->count() > 0)
                <div style="display:flex;flex-direction:column;gap:10px;">
                    @foreach($services as $service)
                        <div style="background:#F4F6F9;border-radius:8px;padding:14px;border-left:3px solid {{ $service->statut === 'actif' ? '#F0A500' : '#ccc' }};">
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                                <div style="font-size:13px;font-weight:700;color:#0D2137;">{{ $service->titre }}</div>
                                <span style="background:{{ $service->statut === 'actif' ? '#FEF3DC' : '#f0f0f0' }};color:{{ $service->statut === 'actif' ? '#7A4500' : '#888' }};padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;">
                                    {{ $service->statut === 'actif' ? '✓ Actif' : '⏸ Suspendu' }}
                                </span>
                            </div>
                            <div style="font-size:12px;color:#5a6a7a;margin-bottom:8px;">
                                {{ $service->categorie->nom ?? '' }} ·
                                {{ $service->tarif ? number_format($service->tarif, 0, ',', ' ').' GNF / séance' : 'À négocier' }}
                            </div>
                            <div style="display:flex;gap:8px;">
                                ...
                                <form method="POST" action="{{ route('service.destroy', $service->id) }}" style="display:inline;" onsubmit="return confirm('Supprimer ce cours ?')">
                                    @csrf @method('DELETE')
            ...
            @else
                <div style="text-align:center;padding:32px;color:#5a6a7a;">
                    ...
                    <div style="font-size:14px;font-weight:600;margin-bottom:6px;">Aucun cours publié</div>
                    <a href="{{ route('service.create') }}" style="background:#F0A500;color:#0D2137;padding:10px 20px;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none;">+ Publier un cours</a>
                </div>
            @endif
        </div>
        ...
        <div style="background:#fff;border-radius:12px;border:1px solid #E2E8F0;padding:20px;margin-bottom:20px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                <h2 style="font-size:15px;font-weight:700;color:#0D2137;">🤝 Demandes d'élèves</h2>
                ...
                <div style="display:flex;flex-direction:column;gap:10px;">
                    @foreach($demandes->take(3) as $demande)
                        <div style="background:#F4F6F9;border-radius:8px;padding:14px;border-left:3px solid {{ $demande->statut === 'en_attente' ? '#F0A500' : ($demande->statut === 'acceptee' ? '#1A9B5A' : '#ccc') }};">
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                                <div style="font-size:13px;font-weight:700;color:#0D2137;">
                                    {{ $demande->demandeur->profile->nom ?? 'Élève' }}
            ...
            @else
                <div style="text-align:center;padding:24px;color:#5a6a7a;">
                    <div style="font-size:13px;">Aucune demande d'élève reçue pour le moment.</div>
                </div>
            @endif
        </div>
                            ...
                            <div style="background:#F4F6F9;border-radius:8px;padding:14px;border-left:3px solid #F0A500;display:flex;align-items:flex-start;justify-content:space-between;gap:10px;">
                                <div style="flex:1;">
                                    <div style="font-size:11px;color:#5a6a7a;margin-bottom:4px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">
                                        {{ class_basename($favori->favorable_type) === 'Service' ? '🎓 Cours' : '💼 Offre' }}
                                    </div>
